...
# v9 Use the real dataset
- [x] Find a plan to modify the preprocessing steps so that it uses the real dataset instead of the dummy one.
adaptation plan:
- [x] **Map Current Flow**  
    - [x] Review preprocess.py, Typer CLI wiring in main.py, and downstream expectations in sft_trainer.py to understand how Alpaca data is being produced and consumed.  
    - [x] Compare outputs (parquet schema, metadata contracts) to those generated by preprocess_drg.py to identify gaps.
- [x] **Design Real-Data Ingestion**  
    - [x] Introduce config knobs in config.yaml / debug_config.yaml (and runtime metadata) to point `preprocess-sft` to raw patient JSON folders (`\\pc-27327\\Projets\\DRG-Prediction\\data\\Extractions\\RAW-JSON-SORTIE`) and optional sample path (sample_real_data).  
    - [x] Plan for a preprocessing stage that first enriches raw letters via `fill_notes.py` (Oracle + CSV) into an intermediate “formatted” directory, then runs the dataset builder mirroring `preprocess_drg.py`.  
    - [x] Decide where enriched JSONs live (e.g., `prepared/real_letters/formatted`) and how run folders capture them for reproducibility.
- [x] **Integrate Oracle Enrichment**  
    - [x] Wrap `fill_notes.update_json_files` into a reusable service under preprocessing with config-driven DSN/user/password (prefer env vars or secrets file) so the CLI can invoke it.  
    - [x] Ensure the enrichment step produces all required fields (`drg_target`, `drg_target_lib`, `predrg_min/max`, dates, etc.) and logs, and handle the “no predrg” quarantine directory per current behavior.  
    - [x] Because DB access is mandatory, add clear failure messaging and possibly a pre-flight check.
- [x] **Reimplement Dataset Builder**  
    - [x] Port logic from preprocess_drg.py into the main pipeline (token length filtering, prefix filtering, rare-label handling, stratified split, label2id stats).  
    - [x] Keep schema identical to the legacy script so training stages work unchanged; make sure optional new metadata columns persist in parquet/JSON as needed.  
    - [x] Provide configuration flags for truncation, prefixes, min tokens, etc., defaulting to values compatible with the existing “dummy” settings.
- [x] **Configuration & CLI Updates**  
    - [x] Extend Typer commands (`preprocess-sft`) to orchestrate: enrich raw → build dataset → write outputs under `prepared/...` with same naming conventions.  
    - [x] Update runtime metadata printer/tests so downstream stages (finetune/export) pick up the new dataset automatically without touching other configs.  
    - [x] Document new config keys and usage in 2.preprocess_sft.md.
- [x] **Testing & Validation**  
    - [x] Add unit/integration tests mirroring existing ones, using sample_real_data plus a stubbed Oracle layer (fixtures that skip external calls but validate enrichment wiring).  
    - [x] Update smoke pipeline to use debug_config.yaml pointing at the sample dataset and a mock Oracle response so CI remains deterministic.  
    - [x] Run local end-to-end dry run to confirm training still launches with the new data.
- [x] **Documentation & Rollout**  
    - [x] Refresh README/docs with “Real dataset” instructions, Oracle prerequisites, and how to swap between sample and full paths.  
    - [x] Provide migration notes for secrets handling (DSN/credentials) and any new environment variables.  
    - [x] Plan a staged rollout: sample debug run → limited subset of production letters → full path after verification.
- [x] Read `src/preprocessing/real_pipeline.py` and `src/cli/main.py` to understand current DRG wiring. *(2025-11-05)*
- [x] Draft follow-up plan for refactor, Oracle enforcement, docs, configuration, and tests. *(2025-11-05 — see latest assistant summary)*
- [x] Refactor `real_pipeline.py` → `drg_pipeline.py` and update all imports/tests/docs. *(2025-11-05)*
- [x] Make Oracle enrichment mandatory for DRG preprocessing (config parsing + runtime checks).
- [x] Fix `config.yaml` typo (`real_data.raw_dir` nesting), propagate to other configs, and document required `.env` vars (`ORACLE_USER`, `ORACLE_PASSWORD`, `ORACLE_NETWORK_ALIAS`, `ORACLE_TNS_PATH`, `ORACLE_DSN`).
- [x] Document preprocessing mode switch (DRG vs Alpaca) across README and docs; ensure Typer examples cover both.
- [x] Restore Alpaca dataset documentation and confirm HuggingFace pathway remains supported.
- [x] Align DRG training objective with classification, mirroring `example_code/finetune_drg.py`; add tests if needed.
- [x] Update tests (e.g., `tests/test_real_preprocess.py`) for mandatory Oracle behaviour and add coverage.
- [ ] Validate container workflow with `debug_config.yaml` end-to-end and capture adjustments.


# small fixes, on the go
- [x] add metadata to each log folder. So we have all info about the configs used in detailled for reproducibility purposes and info about the parameters used for each step.
...
- [x] on tensorboard, we observe that each run has the name of the log_vxx/train/tensorboard folder, we should fix that so that we have the model_name-dataset_name-dataset_size_runX
