"""TensorBoard integration smoke test (clean copy)."""
...
# Provide lightweight shims for optional packages that may not be
# available in the test container (peft / awq / trl). These prevent
# top-level imports from failing when importing the CLI module below.
import sys
import types
...
class _StubTrainer:
    ...
    def train(self, resume_from_checkpoint=None):  # pylint: disable=unused-argument
        # Trainer may or may not include `logging_dir` depending on the
        # Transformers version. Prefer it when present, otherwise try to
        # locate an existing `tensorboard` directory created by the
        # runner (under the test tmp dir). As a final fallback, derive a
        # plausible path from the TrainingArguments.output_dir.
        logdir = None
        if hasattr(self.args, "logging_dir") and self.args.logging_dir:
            logdir = Path(self.args.logging_dir)
        else:
            # Search common temp locations for a 'tensorboard' folder created
            # by the runner (this lives under the test tmp_path which is in /tmp).
            for candidate in Path("/tmp").rglob("tensorboard"):
                if candidate.is_dir():
                    logdir = candidate
                    break
        if logdir is None:
            # Fallback: guess based on output_dir -> trainer_state parent
            try:
                guess = Path(self.args.output_dir).parents[1] / "tensorboard"
                logdir = guess
            except Exception:
...
def test_tensorboard_events_written(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, runner: CliRunner) -> None:
    ...
    # Provide a minimal AutoTokenizer-like class with a from_pretrained classmethod
    class DummyAutoTokenizer:
        @classmethod
        def from_pretrained(cls, *args, **kwargs):
            return _DummyTokenizer()

    monkeypatch.setattr("src.data.preprocess.AutoTokenizer", DummyAutoTokenizer)
    monkeypatch.setattr("src.training.sft_trainer.AutoTokenizer", DummyAutoTokenizer)
    class DummyAutoModelForCausalLM:
        @classmethod
        def from_pretrained(cls, *args, **kwargs):
            return object()

    monkeypatch.setattr("src.training.sft_trainer.AutoModelForCausalLM", DummyAutoModelForCausalLM)
