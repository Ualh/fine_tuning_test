...
Launch an OpenAI-compatible vLLM server on the merged or AWQ-compressed model.

## What it does
Starts the `vllm-server` container. When included in `orchestration.post_finetune`, the wrapper starts vLLM automatically after evaluation using the same run’s resolved served model.
- Serves the model via OpenAI API on `http://localhost:8080`
