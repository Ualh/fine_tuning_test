...
The `vllm-server` service mounts `./outputs` as `/models`, and the CLI resolves which subfolder to serve.
By default, when `serve.prefer_awq: true`, vLLM will serve `<run>/merged_awq` if present, otherwise `<run>/merged`.
