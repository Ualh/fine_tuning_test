...
## One-time setup
1) Create a `.env` file with credentials (copy this template and fill in your values):

```
HF_TOKEN=hf_XXXXXXXXXXXXXXXXXXXXXXXX        # optional, required for gated models
ORACLE_USER=XXXXXXXXX
ORACLE_PASSWORD=XXXXXXXXX
ORACLE_DSN=host:port/service_name          # EZConnect string
ORACLE_NETWORK_ALIAS=PR_DWUNI              # alias defined in tnsnames.ora
ORACLE_TNS_PATH=C:\path\to\tnsnames\directory
```
...
### preprocess
- `dataset_name`: logical dataset identifier (e.g., `drg_letters`). CLI override: `--dataset-name`.
