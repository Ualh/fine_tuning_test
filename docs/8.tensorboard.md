...
When you run the finetune stage, event files are written under the per-run directory `logs/<run-name>/train/tensorboard/`.

Note: On some `transformers/trl` versions, the Trainer may ignore a custom `logging_dir` and write under `outputs/<run>/trainer_state/runs/...` instead. This repo includes a small fallback writer to always mirror core metrics into `logs/<run>/train/tensorboard/` so the dashboard remains consistent.

The wrapper prints the active logdir:
...
```
[FINETUNE] TensorBoard=http://localhost:6006 (logdir=logs/qwen2-7b-drg_letters-full-run3/train/tensorboard)
```
...
- Verify event files exist: `ls logs/*-run*/train/tensorboard/*.tfevents*` inside the repo or container. If empty, also check `outputs/*-run*/trainer_state/runs/` for event files.
